...
#include "Chipset_4081.hpp"

namespace nts {

    Chipset_4081::Chipset_4081(const std::string &name) :
        _name(name),
        _type("4081")
    {
        ...
        IPin *pin3 = std::make_unique<Pin_and>(pin1, pin2).release();
        IPin *pin4 = std::make_unique<Pin_and>(pin5, pin6).release();
        IPin *pin7 = NULL;
        ...
        IPin *pin10 = std::make_unique<Pin_and>(pin8, pin9).release();
        IPin *pin11 = std::make_unique<Pin_and>(pin12, pin13).release();
        IPin *pin14 = NULL;
    ...
    Chipset_4081::~Chipset_4081()
    {
        for (int i = 0; i < (int)_pins.size(); i++)
            delete _pins[i];
    }

    Tristate Chipset_4081::compute(std::size_t pin)
    {
        if (pin < 1 || pin > 14) return UNDEFINED;
        return _pins[pin-1]->compute();
    }
    void Chipset_4081::setLink(std::size_t pin, nts::IComponent *other, std::size_t otherPin)
    {
        _pins[pin-1]->setLink(other->getPin(otherPin));
    }
    void Chipset_4081::dump() const
    {}
    void Chipset_4081::simulate(std::size_t tick)
    {}

    std::string Chipset_4081::getName()
    {
        return _name;
    }

    std::string Chipset_4081::getType()
    {
        return _type;
    }

    void Chipset_4081::setValue(Tristate value)
    {}

    void Chipset_4081::printInfo(int num)
    {
        std::cout << "My name is : " << _name << std::endl << "My type is : " << _type << std::endl << "----------" << std::endl;
    }

    IPin *Chipset_4081::getPin(size_t pin)
    {
